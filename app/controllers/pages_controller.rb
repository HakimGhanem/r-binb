    ...
    def home
        ...
        def search
          if params[:search].present? && params[:search].strip != ""
             session[:r_binb_search] = params[:search]
          end
          
          arrResult = Array.new
          
          if session[:aiirbnb_search] && session[:r_binb_search] != ""
             @rooms_address = Room.where(active: true).near(session[:r_binb_search], 5, order:'distance')
          else
